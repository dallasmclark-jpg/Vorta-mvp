...
function BookingDrawer({
  ...
  const canReject =
    item?.status === "pending_approval" || item?.status === "booked";
    ...
    <DetailDrawer open={isOpen} onClose={onClose}>
      ...
      <div className="flex items-start justify-between border-b border-gray-800 p-5">
        <div className="flex min-w-0 flex-1 flex-col gap-1.5 pr-3">
          <div className="flex flex-wrap items-center gap-2">
            {item && (
              <Badge
                className={`inline-flex h-auto rounded px-2 py-0.5 text-[10px] font-medium shadow-none ${bookingStatusBadgeClass(item.status)}`}
              >
            ...
            {item?.delivery_type && (
              <Badge
                className={`inline-flex h-auto rounded px-1.5 py-0.5 text-[10px] font-medium shadow-none ${deliveryBadgeClass(item.delivery_type)}`}
              >
                {item.delivery_type}
              </Badge>
            )}
          ...
          {item?.partner_name && (
            <p className="text-sm text-slate-400">{item.partner_name}</p>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-gray-800 border-b border-gray-800">
        {[
          ...
          {
            label: "Date",
            value: fmtDate(item?.booking_date ?? null),
            cls: "text-slate-200",
          },
        ].map(({ label, value, cls }) => (
        ...
        <div className="border-b border-gray-800 p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              {
                label: "Cost",
                value: item?.cost ? fmtCurrency(item.cost, item.currency) : "—",
              },
              ...
              ...(item?.duration_days
                ? [
                    {
                      label: "Duration",
                      value: `${item.duration_days} day${item.duration_days !== 1 ? "s" : ""}`,
                    },
                  ]
                : []),
            ].map(({ label, value }) => (
          ...
          <div className="border-b border-gray-800 p-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {item!.skills_covered!.map((s) => (
                <span
                  key={s}
                  className="rounded-full border border-gray-700 bg-[#111620] px-2.5 py-0.5 text-[11px] font-medium text-slate-300"
                >
                  {s}
                </span>
              ))}
...
export const TrainingSection = (): JSX.Element => {
  ...
  const [selectedItem, setSelectedItem] = useState<BookingDrawerItem | null>(
    null,
  );
  ...
  const filteredPriority = useMemo(() => {
    ...
    return priorityRows.filter((r) => {
      if (
        prioritySearch &&
        !r.skill_name.toLowerCase().includes(lc) &&
        !(r.dept_name ?? "").toLowerCase().includes(lc)
      )
        return false;
  ...
  const totalPriorityPages = Math.ceil(
    filteredPriority.length / PRIORITY_PAGE_SIZE,
  );
  const pagedPriority = filteredPriority.slice(
    priorityPage * PRIORITY_PAGE_SIZE,
    (priorityPage + 1) * PRIORITY_PAGE_SIZE,
  );

  const maxSpend = useMemo(
    () => Math.max(...spendByMonth.map((s) => s.spend), 1),
    [spendByMonth],
  );
    ...
    () => [
      {
        ...
        icon: GraduationCap,
        valueClass:
          stats.engineersNeedingTraining > 0
            ? "text-orange-400"
            : "text-emerald-400",
      },
      {
      ...
      },
      {
        ...
        icon: Shield,
        valueClass:
          stats.expiringIn30Days > 0 ? "text-orange-400" : "text-emerald-400",
      },
      {
        label: "Spend YTD",
    ...
    <section className="relative flex min-w-0 w-full max-w-full flex-1 grow flex-col items-start gap-6 overflow-x-hidden px-4 pb-12 pt-0 md:gap-8 md:px-6 xl:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <ContextHelp
              content={{
                ...
                usage:
                  "Use Add Booking to schedule training. Filter by status or engineer to find specific records. Export to share training plans with management.",
                aiNote:
                  "Vorta AI ranks engineers for training based on skill gaps, SPOF risk and equipment criticality — helping you prioritise training investment.",
              }}
            />
          </div>
          <p className="text-sm text-slate-400">
            Prioritise training gaps, compliance risks and development needs
            across your maintenance team.
          </p>
        </div>
      ...
      <div className="flex w-full flex-col gap-4">
        <SyncIndicator
          ...
          source="Supabase"
          confidence={
            stats.compliancePct > 0
              ? Math.min(95, Math.round(stats.compliancePct * 0.9 + 10))
              : undefined
          }
        />
        {!loading && !loadError && (
          <AiActionsPanel
            actions={
              [
                {
                  label: "Book priority training now",
                  description: `${stats.criticalGaps} critical skill gap${stats.criticalGaps !== 1 ? "s" : ""} require immediate training. AI has identified engineers and ranked recommended courses.`,
                  priority: "critical",
                  icon: GraduationCap,
                  href: "/training-providers",
                },
                {
                  label: "Renew expiring certifications",
                  description: `${stats.expiringIn30Days} certification${stats.expiringIn30Days !== 1 ? "s" : ""} expire within 30 days. Book renewals before skills lapse and create compliance risk.`,
                  priority: stats.expiringIn30Days > 0 ? "high" : "medium",
                  icon: AlertTriangle,
                },
                {
                  label: "Create training plan for the quarter",
                  description:
                    "Use AI Matching to generate a prioritised training plan based on skill gaps, equipment risk and compliance deadlines.",
                  priority: "medium",
                  icon: BookOpen,
                  href: "/ai-matching",
                },
                {
                  label: "Review training spend vs ROI",
                  description: `£${stats.totalSpendGBP.toLocaleString()} spent on training to date. Compare this against risk reduction and compliance improvements.`,
                  priority: "low",
                  icon: TrendingUp,
                },
              ] as AiAction[]
                    ...
                    {loading ? (
                      ...
                      insights.map((ins, i) => {
                        ...
                        return (
                          ...
                          >
                            <Icon
                              className={`mt-0.5 h-4 w-4 shrink-0 ${conf.iconCls}`}
                            />
                            <div className="min-w-0 flex-1">
                              <p
                                className={`text-sm font-semibold ${conf.titleCls}`}
                              >
                                {ins.title}
                              </p>
                  ...
                  {loading ? (
                    <div className="flex items-end gap-2 h-24">
                      {Array.from({ length: 6 }).map((_, i) => (
                        <div
                          key={i}
                          className="flex-1 animate-pulse rounded-sm bg-gray-800"
                  ...
                  <div className="border-t border-gray-800 pt-3">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-sm font-semibold tabular-nums text-slate-200">
                        {loading ? "—" : fmtCurrency(stats.totalSpendGBP)}
                      </span>
                    </div>
                      ...
                      {loading
                        ...
                        : bookingsByDept.map((d) => (
                            <div key={d.dept} className="flex flex-col gap-1">
                              ...
                              <Progress
                                value={d.pct}
                                className={`h-1.5 overflow-hidden rounded bg-gray-800 ${coverageBarClass(d.pct)}`}
                              />
                            </div>
                          ))}
                    </div>
                ...
                <CardContent className="flex min-w-0 flex-col gap-4 p-5">
                  ...
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center gap-2 py-8 text-center">
                      ...
                      <p className="text-xs text-slate-500">
                        No certifications expiring within 90 days.
                      </p>
                    </div>
                  ) : (
                      ...
                      {certRiskRows.map((r, i) => (
                        ...
                        >
                          ...
                          <div className="flex shrink-0 flex-col items-end gap-1">
                            <Badge
                              className={`inline-flex h-auto rounded px-2 py-0.5 text-[10px] font-medium shadow-none ${certStatusBadgeClass(r.status)}`}
                            >
                              {r.status}
                            </Badge>
                            <span className="text-[10px] text-slate-500">
                              {r.days_left < 0
                                ? `${Math.abs(r.days_left)}d overdue`
              ...
              <Card className="min-w-0 rounded-xl border border-gray-800 bg-[#141820] shadow-none">
                <CardContent className="flex min-w-0 flex-col gap-4 p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-slate-500">
                      {loading ? "—" : `${recommendedCourses.length} active`}
                    </span>
                  </div>
                  {loading ? (
                    <div className="flex flex-col gap-3">
                        ...
                        >
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2">
                              ...
                              {c.delivery_type && (
                                <Badge
                                  className={`inline-flex h-auto rounded px-1.5 py-0.5 text-[9px] font-medium shadow-none ${deliveryBadgeClass(c.delivery_type)}`}
                                >
                                  {c.delivery_type}
                                </Badge>
                              )}
              ...
              <CardContent className="flex min-w-0 flex-col gap-4 p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-400">
                      {loading
                        ? "Loading…"
                        : `${filteredPriority.length} of ${priorityRows.length} skill gaps`}
                      {totalPriorityPages > 1
                        ? ` · page ${Math.min(priorityPage + 1, totalPriorityPages)} of ${totalPriorityPages}`
                        : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        type="button"
                        onClick={() =>
                          setPriorityPage((p) => Math.max(0, p - 1))
                        }
                        disabled={priorityPage === 0 || loading}
                      ...
                      <button
                        ...
                        onClick={() =>
                          setPriorityPage((p) =>
                            Math.min(totalPriorityPages - 1, p + 1),
                          )
                        }
                        disabled={
                          priorityPage >= totalPriorityPages - 1 || loading
                        }
                        className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-800 text-slate-400 transition-colors hover:bg-[#ffffff1a] hover:text-slate-200 disabled:opacity-30"
                      >
                ...
                <div className="w-full max-w-full overflow-x-auto rounded-lg border border-gray-800">
                  <table className="w-max min-w-[740px] border-collapse text-sm">
                    ...
                    <tbody>
                      {loading ? (
                        Array.from({ length: PRIORITY_PAGE_SIZE }).map(
                          (_, i) => (
                            <tr
                              key={i}
                        ...
                        <tr>
                          ...
                          >
                            No training gaps found. Your team meets the selected
                            requirements.
                          </td>
                        </tr>
                      ) : (
                        pagedPriority.map((row, idx) => {
                          ...
                          return (
                            <tr
                              key={row.id}
                              className={`border-b border-gray-800/50 ${rowBg} transition-colors hover:bg-[#1a2030]`}
                            >
                              <td
                                className={`sticky left-0 z-10 min-w-[200px] px-4 py-2.5 ${rowBg}`}
                              >
                              ...
                              <td className="px-4 py-2.5 text-center">
                                <span
                                  className={`text-sm font-semibold tabular-nums ${row.current_avg >= row.target_rating ? "text-emerald-400" : "text-yellow-400"}`}
                                >
                              ...
                              <td className="px-4 py-2.5">
                                <Badge
                                  className={`inline-flex h-auto rounded px-2 py-0.5 text-[10px] font-medium shadow-none ${priorityBadgeClass(row.priority)}`}
                                >
                                  {row.priority}
                                </Badge>
                              </td>
                ...
                {!loading && totalPriorityPages > 1 && (
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>
                      {priorityPage * PRIORITY_PAGE_SIZE + 1}–
                      {Math.min(
                        (priorityPage + 1) * PRIORITY_PAGE_SIZE,
                        filteredPriority.length,
                      )}{" "}
                      of {filteredPriority.length}
                    </span>
                    <div className="flex items-center gap-1">
                      {Array.from({
                        length: Math.min(totalPriorityPages, 8),
                      }).map((_, i) => (
                        <button
                          key={i}
                          type="button"
                          onClick={() => setPriorityPage(i)}
                ...
                <CardContent className="flex min-w-0 flex-col gap-4 p-5">
                  ...
                  <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {trainingPartners.map((p) => (
                      <div
                        key={p.id}
                        className="flex flex-col gap-3 rounded-lg border border-gray-800 bg-[#0b0e14] p-4"
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <Badge
                            className={`inline-flex h-auto shrink-0 rounded px-2 py-0.5 text-[10px] font-medium shadow-none ${p.status === "active" ? "bg-[#10b98120] text-emerald-500" : "bg-gray-800 text-slate-400"}`}
                          >
                            {p.status}
                          </Badge>
                        </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      Latest activity · full management on Bookings page
                    </p>
                  </div>
                  <span className="text-[11px] text-slate-500">
                    {loading ? "—" : `${recentActivity.length} shown`}
                  </span>
                </div>
                ...
                {loading ? (
                  <div className="flex flex-col">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 border-b border-gray-800/60 py-2.5"
                      >
                    ...
                    {recentActivity.slice(0, 8).map((b) => {
                      ...
                      return (
                        <div
                          key={b.id}
                          onClick={() =>
                            setSelectedItem({ ...b, status: effectiveStatus })
                          }
                          className="flex cursor-pointer items-center gap-3 py-2.5 transition-colors hover:bg-[#ffffff04]"
                        >
                          <div
                            className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[9px] font-semibold ${avatarColor(b.engineer_name)}`}
                          >
                          ...
                          <div className="flex shrink-0 items-center gap-3">
                            ...
                            <Badge
                              className={`inline-flex h-auto rounded px-1.5 py-0.5 text-[9px] font-medium shadow-none ${bookingStatusBadgeClass(effectiveStatus)}`}
                            >
                              {verb}
                            </Badge>
                          </div>
