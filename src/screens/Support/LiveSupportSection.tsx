...
function statusClass(status: string): string {
  ...
  if (["matched", "accepted", "in_progress"].includes(value)) {
    return "border-blue-500/30 bg-blue-500/10 text-blue-300";
  }
  if (["open", "pending"].includes(value)) {
  ...
  const load = useCallback(async (initial = false): Promise<void> => {
    ...
    try {
      ...
      if (requestError || !data) {
        throw requestError ?? new Error("Support evidence was empty");
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Verified support evidence is unavailable.",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
      ...
      <header className="flex flex-col justify-between gap-4 py-5 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-slate-400">
            Active-site equipment support requests, matching, sessions and resolution reports. Platform support remains a direct Vorta contact, not a simulated ticket queue.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <a
            href="mailto:support@vorta.network?subject=Vorta%20pilot%20support"
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 text-sm font-semibold text-blue-200 transition-colors hover:bg-blue-500/20"
          >
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/10 px-4 text-sm font-semibold text-slate-100 transition-colors hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      {!loading && error ? (
        <div className="flex flex-col justify-between gap-4 rounded-xl border border-red-500/30 bg-red-500/10 p-5 sm:flex-row sm:items-center" role="alert">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-100">Support evidence was withheld</p>
      ...
      {!loading && payload ? (
        <>
          ...
          <Card className="rounded-xl border border-gray-800 bg-[#141820] shadow-none">
            <CardContent className="p-0">
              <div className="border-b border-gray-800 p-5">
                <h2 className="font-semibold text-slate-50">Operational support register</h2>
                <p className="mt-1 text-xs text-slate-500">Evidence only. This page cannot create, reply to, match or close support requests.</p>
              </div>
              <div className="grid gap-4 p-4 xl:grid-cols-2">
                {requests.length === 0 ? (
                  <p className="p-1 text-sm text-slate-500">No site-attributable support requests are recorded.</p>
                ) : requests.map((request) => (
                  <article key={request.id} className="rounded-xl border border-gray-800 bg-[#111620] p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-100">{request.requestTitle}</p>
                        <p className="mt-0.5 text-xs text-slate-500">
                          {request.equipmentName ?? "Equipment not recorded"}
                          {request.departmentName ? ` · ${request.departmentName}` : ""}
                        </p>
                      </div>
                      <div className="flex shrink-0 flex-col items-end gap-1.5">
                        <Badge className={`h-auto rounded border px-2 py-0.5 text-[10px] shadow-none ${priorityClass(request.priority)}`}>
                          {request.priority}
                        </Badge>
                        <Badge className={`h-auto rounded border px-2 py-0.5 text-[10px] shadow-none ${statusClass(request.status)}`}>
                    ...
                    {request.issueDescription ? (
                      <p className="mt-3 text-xs leading-5 text-slate-400">{request.issueDescription}</p>
                    ) : null}

                    {request.productionStopped ? (
                      <div className="mt-3 flex items-center gap-2 rounded-lg border border-red-500/25 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-200">
                        <ShieldAlert className="h-4 w-4" aria-hidden="true" />
                        Production stopped{request.estimatedDowntimeMinutes ? ` · estimated ${request.estimatedDowntimeMinutes} minutes` : ""}
                      </div>
                    ) : null}

                    <div className="mt-4 grid grid-cols-3 gap-3 text-xs">
                      <div><p className="text-slate-600">Matches</p><p className="mt-0.5 font-semibold text-slate-300">{request.matchCount}</p></div>
                      <div><p className="text-slate-600">Sessions</p><p className="mt-0.5 font-semibold text-slate-300">{request.sessionCount}</p></div>
                      <div><p className="text-slate-600">Reports</p><p className="mt-0.5 font-semibold text-slate-300">{request.reportCount}</p></div>
                    </div>

                    {request.skillNames.length > 0 ? (
                      <div className="mt-4 flex flex-wrap gap-1.5">
                        {request.skillNames.slice(0, 5).map((skill) => (
                          <span key={skill} className="rounded-md border border-slate-700 bg-slate-800/60 px-2 py-1 text-[10px] text-slate-300">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="mt-4 grid gap-2 text-xs sm:grid-cols-2">
                      <div className="flex items-center gap-2 text-slate-500">
                        ...
                        Opened {formatDateTime(request.openedAt)}
                      </div>
                      <div className="flex items-center gap-2 text-slate-500 sm:justify-end">
                        {request.requiredSupportType?.includes("video") ? <Video className="h-3.5 w-3.5" aria-hidden="true" /> : <Wrench className="h-3.5 w-3.5" aria-hidden="true" />}
                        {request.requiredSupportType?.replaceAll("_", " ") ?? "Support type not recorded"}
                      </div>
                    </div>

                    {request.latestReportTitle || request.resolutionSummary ? (
                      <div className="mt-4 rounded-lg border border-emerald-500/20 bg-emerald-500/[0.07] p-3">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-300">Recorded outcome evidence</p>
                        {request.latestReportTitle ? <p className="mt-1 text-xs font-medium text-slate-200">{request.latestReportTitle}</p> : null}
                        {request.resolutionSummary ? <p className="mt-1 text-xs leading-5 text-slate-400">{request.resolutionSummary}</p> : null}
                      </div>
                    ) : null}
                  </article>
                ))}
          ...
          <div className="rounded-xl border border-amber-500/25 bg-amber-500/[0.07] p-5">
            <div className="flex items-start gap-3">
              <Mail className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" aria-hidden="true" />
              <div>
                <p className="text-sm font-semibold text-amber-100">Vorta platform support is handled directly</p>
                <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-400">
                  The application does not yet persist platform support tickets. Email support@vorta.network so the request enters a real support channel rather than a browser-only queue.
                </p>
              </div>
            </div>
          </div>
