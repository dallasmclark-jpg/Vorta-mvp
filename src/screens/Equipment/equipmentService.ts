...
function resolvePlaceholderImage(name: string | null, type: string | null, code: string | null): string {
  const n = (name ?? "").toLowerCase();
  const t = (type ?? "").toLowerCase();
  const c = (code ?? "").toUpperCase();
  const isVialFiller =
    n.includes("vial") || n.includes("filler") || n.includes("filling") ||
    t.includes("vial") || t.includes("filler") || t.includes("filling") ||
    c.startsWith("VF") || c.startsWith("FL");
  const isPalletiser =
    n.includes("palletis") || n.includes("palletiz") ||
    t.includes("palletis") || t.includes("palletiz") ||
    c.startsWith("PL");
  if (isVialFiller) return VIAL_FILLER_IMAGE;
  if (isPalletiser) return PALLETISER_IMAGE;
  return PALLETISER_IMAGE;
}
...
function rowToEquipment(row: EquipmentAssetRow): Equipment {
  ...
  return {
    ...
    statusNote:   "",
    image:        row.image_url || resolvePlaceholderImage(row.name, row.equipment_type, row.equipment_code),
    riskScore,
    riskLevel,
