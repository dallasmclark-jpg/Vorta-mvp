...
import { AlertTriangle, ChevronRight, Filter, Search, Wrench } from "lucide-react";
...
type RiskFilter = "all" | "critical-high" | "overdue";
type SortKey = "risk" | "backlog" | "name";
...
export function MobileEquipmentSection(): JSX.Element {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const requestedArea = searchParams.get("area")?.trim() || searchParams.get("building")?.trim() || "all";
  ...
  const load = async (): Promise<void> => {
    ...
    setError(null);
    try {
      const next = await getEquipmentList();
      setItems(next);
    } catch (loadError) {
      setItems([]);
      setError(loadError instanceof Error ? loadError.message : "Equipment records could not be loaded.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    return [...items]
      .filter((item) => {
        ...
        if (riskFilter === "critical-high" && !["Critical", "High"].includes(item.riskLevel)) return false;
        if (riskFilter === "overdue" && item.overduePmCount + item.calibrationOverdueCount === 0) return false;
        return true;
      })
      .sort((left, right) => {
  ...
  }, [area, items, riskFilter, search, sortKey]);

  const atRiskCount = items.filter((item) => ["Critical", "High"].includes(item.riskLevel)).length;
  const overdueCount = items.reduce(
    (total, item) => total + item.overduePmCount + item.calibrationOverdueCount,
    0,
  );
  const activeFilterCount = Number(area !== "all") + Number(riskFilter !== "all") + Number(sortKey !== "risk");
    ...
    >
      <DetailDrawer open={filtersOpen} onClose={() => setFiltersOpen(false)}>
        <div className="flex items-start justify-between border-b border-gray-800 p-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-blue-300">Equipment filters</p>
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-5">
          <fieldset>
            <legend className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Area</legend>
          ...
          <fieldset>
            <legend className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Risk</legend>
            <div className="mt-3 grid gap-2">
              {([
                ["all", "All risk states"],
          ...
          <fieldset>
            <legend className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Sort</legend>
            <div className="mt-3 grid gap-2">
              {([
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-xl border border-gray-800 bg-[#141820] p-3">
          ...
          <p className="mt-1 text-xl font-semibold text-slate-50">{loading ? "—" : items.length}</p>
        </div>
        <div className="rounded-xl border border-orange-500/20 bg-orange-500/[0.05] p-3">
          ...
          <p className="mt-1 text-xl font-semibold text-orange-300">{loading ? "—" : atRiskCount}</p>
        </div>
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.05] p-3">
          <p className="text-[10px] text-slate-500">Overdue</p>
          <p className="mt-1 text-xl font-semibold text-amber-300">{loading ? "—" : overdueCount}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-1.5 rounded-md border border-gray-800 bg-[#141820] px-2 py-1 text-[10px] font-semibold text-slate-400">
          <Wrench className="h-3 w-3" aria-hidden="true" /> Mobile view
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {loading && items.length === 0
          ? Array.from({ length: 4 }, (_, index) => (
              <div key={index} className="h-40 animate-pulse rounded-xl border border-gray-800 bg-[#141820]" />
            ))
          : filtered.map((item) => {
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 text-right">
                      <p className={`text-xl font-semibold tabular-nums ${riskTone(item.riskLevel).split(" ").at(-1)}`}>{item.riskScore.toFixed(1)}</p>
                      <span className={`mt-1 inline-flex rounded-md border px-2 py-0.5 text-[10px] font-semibold ${riskTone(item.riskLevel)}`}>
                        {item.riskLevel}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3 border-t border-gray-800 pt-3">
                    ...
                    <span className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-blue-300">
      ...
      {!loading && filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-800 bg-[#10151d] px-6 py-10 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500">Clear the search or filters to restore the list.</p>
        </div>
      ) : null}
    </section>
  );
