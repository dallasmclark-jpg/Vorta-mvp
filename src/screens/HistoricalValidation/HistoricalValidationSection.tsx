...
import type { HistoricalBacktestCase } from "../Equipment/equipmentHistoricalBacktestService";
import {
...
import type { HistoricalBacktestResult } from "../Equipment/equipmentHistoricalBacktestService";
...
function outcomeDate(item: HistoricalBacktestCase): string {
  return formatDate(
...
function MetricCard({ metric }: { metric: MetricDefinition }): JSX.Element {
  ...
  return (
    <article
      ...
      className="min-h-[118px] rounded-xl border border-gray-800 bg-[#141820] p-4"
    >
    ...
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-blue-500/25 bg-blue-500/[0.06] px-2 py-1 text-xs font-semibold text-blue-200">
              {scenarioLabel(item)}
            </span>
          ...
          <h3 className="mt-3 text-base font-semibold text-slate-50 sm:text-lg">
            {item.equipment.name}
          </h3>
        ...
        <button
          ...
          className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg border border-gray-700 bg-[#0d1117] px-3 py-2 text-sm font-semibold text-slate-200 transition-colors hover:border-gray-600 hover:bg-gray-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/60"
        >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-gray-800 bg-[#0d1117] p-3">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-400">{outcomeDate(item)}</p>
        </div>
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {Array.from({ length: 8 }).map((_, index) => (
          <div
            key={index}
            className="h-[118px] animate-pulse rounded-xl border border-gray-800 bg-[#141820]"
          />
        ))}
      </div>
...
export function HistoricalValidationSection(): JSX.Element {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const nextResult = await loadHistoricalValidation(siteContext.siteId);
  ...
  useEffect(() => {
    if (scope === "all") return;
    if (!areas.some((item) => item.area === scope)) {
      setScope("all");
    }
  }, [areas, scope]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="rounded-xl border border-violet-500/20 bg-violet-500/[0.05] px-4 py-3"
      >
      ...
      <div
        className="overflow-x-auto border-b border-gray-800 pb-3"
      ...
      {scopedResult && scopedResult.status === "empty" ? (
