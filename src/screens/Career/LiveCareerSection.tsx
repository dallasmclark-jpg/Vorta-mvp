...
export function LiveCareerSection(): JSX.Element {
  ...
  const load = useCallback(async (initial = false): Promise<void> => {
    ...
    try {
      ...
      if (requestError || !data) {
        throw requestError ?? new Error("Career evidence was empty");
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Verified career evidence is unavailable.",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const requirements = useMemo(
    () => payload?.requirements.filter((row) => !["complete", "completed", "met", "verified"].includes(row.status.toLowerCase())).slice(0, 10) ?? [],
    [payload],
  );
      ...
      <header className="flex flex-col justify-between gap-4 py-5 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-slate-400">
            Active-site career paths, readiness and development requirements. This is workforce evidence, not a personal profile for the signed-in manager.
          </p>
        </div>
        <button
          type="button"
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              ["Average readiness", `${payload.stats.averageReadiness.toFixed(0)}%`, TrendingUp, readinessClass(payload.stats.averageReadiness)],
          ...
          <Card className="rounded-xl border border-gray-800 bg-[#141820] shadow-none">
            <CardContent className="p-0">
              <div className="border-b border-gray-800 p-5">
                ...
                <p className="mt-1 text-xs text-slate-500">Readiness and progress are recorded workforce evidence. No career decisions are written from this page.</p>
              </div>
              <div className="grid gap-4 p-4 lg:grid-cols-2">
                {paths.length === 0 ? (
                  <p className="p-1 text-sm text-slate-500">No active career paths are recorded for this site.</p>
                ) : paths.map((path) => {
                  const completed = path.completedRequirementCount;
                  const total = path.requirementCount;
                  return (
                    <article key={path.id} className="rounded-xl border border-gray-800 bg-[#111620] p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge className="h-auto shrink-0 rounded border border-blue-500/25 bg-blue-500/10 px-2 py-0.5 text-[10px] text-blue-300 shadow-none">
                      ...
                      <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-xs">
                        <div>
                          ...
                          <p className="mt-1 font-medium text-slate-300">{path.currentJobRole}</p>
                        </div>
                        <TrendingUp className="h-4 w-4 text-slate-600" aria-hidden="true" />
                        <div className="text-right">
                          <p className="text-slate-600">Target</p>
                          <p className="mt-1 font-medium text-slate-200">{path.targetJobRole}</p>
                        </div>
                      </div>
                      <div className="mt-4">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-slate-500">Readiness</span>
                          <span className={`font-semibold ${readinessClass(path.readinessScore)}`}>{path.readinessScore.toFixed(0)}%</span>
                        </div>
                        <div className="mt-2 h-2 overflow-hidden rounded-full bg-gray-800">
                          <div className={`h-full rounded-full ${readinessBarClass(path.readinessScore)}`} style={{ width: `${path.readinessScore}%` }} />
                        </div>
                      </div>
                      <div className="mt-4 grid grid-cols-3 gap-3 text-xs">
                        <div><p className="text-slate-600">Requirements</p><p className="mt-0.5 font-semibold text-slate-300">{completed}/{total}</p></div>
                      ...
                      <div className="mt-3 flex items-center gap-2 text-xs text-slate-600">
                        ...
                        {path.estimatedTimeframe ?? "Timeframe not recorded"} · updated {formatDate(path.updatedAt)}
                      </div>
                    </article>
                  );
                })}
          ...
          <Card className="rounded-xl border border-gray-800 bg-[#141820] shadow-none">
            <CardContent className="p-0">
              <div className="border-b border-gray-800 p-5">
                <h2 className="font-semibold text-slate-50">Open development requirements</h2>
                <p className="mt-1 text-xs text-slate-500">Highest-priority incomplete evidence items across active paths.</p>
              </div>
              <div className="divide-y divide-gray-800/80">
                {requirements.length === 0 ? (
                  <p className="p-5 text-sm text-slate-500">No incomplete development requirements are recorded.</p>
                  ...
                  <div key={row.id} className="flex flex-col justify-between gap-3 p-4 sm:flex-row sm:items-start">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-slate-500">{row.engineerName} · {row.requirementType.replaceAll("_", " ")}</p>
                      {row.notes ? <p className="mt-2 text-xs leading-5 text-slate-400">{row.notes}</p> : null}
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                      {row.currentLevel !== null && row.targetLevel !== null ? (
                        <span className="text-xs font-semibold text-slate-400">{row.currentLevel} → {row.targetLevel}</span>
                      ) : null}
