...
test("capture Maintenance Manager visual quality audit", async ({ page }, testInfo) => {
  ...
  if (width < 640) {
    ...
    const launcher = page.getByRole("button", { name: "Ask Vorta AI", exact: true });
    await launcher.waitFor({ state: "visible", timeout: 15_000 });
    await launcher.click();
  }
  await page.locator('[data-vorta-global-ai-panel="true"]:visible, [data-vorta-ai-workspace="true"]:visible').waitFor({ state: "visible", timeout: 15_000 });
