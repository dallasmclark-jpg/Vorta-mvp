...
export async function context(req: Request): Promise<{
  ...
  if (!url || !key) throw { status: 500, message: "Function configuration is incomplete" };
  ...
  const { data, error } = await db.rpc("vorta_get_function_context").maybeSingle();
  if (error) throw { status: 403, message: "Portal access could not be verified" };
