...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const engineerId = typeof payload?.engineerId === "string" ? payload.engineerId : "";
    const assessmentLevel = Number(payload?.assessmentLevel);
    const evidenceReference = typeof payload?.evidenceReference === "string" ? payload.evidenceReference.slice(0, 1000) : null;
    ...
    if (targetError || !targetEngineer || targetEngineer.site_id !== equipment.site_id || targetEngineer.organisation_id !== profile.organisation_id) {
      return json(req, { error: "Target engineer is not assigned to this site" }, 400);
    }

    const { data: accessRows, error: accessError } = await db.from("user_site_access")
      .select("site_id,organisation_id,app_role,active")
      .eq("user_id", user.id)
      .eq("site_id", equipment.site_id)
    ...
    const { data: assessorEngineer } = await db.from("engineers")
      .select("id,full_name,site_id,organisation_id")
      .eq("profile_id", user.id)
      .eq("site_id", equipment.site_id)
      .eq("organisation_id", profile.organisation_id)
      .maybeSingle();

    if (assessorEngineer?.id === engineerId) {
      return json(req, { error: "Engineers cannot validate their own equipment competency" }, 403);
    }
    ...
    if (access && MANAGER_ROLES.has(role)) {
      ...
      const { data: peerCapability, error: peerError } = await db.from("equipment_engineer_capabilities")
        .select("competency_level,capability_status,validation_status,practice_authority,valid_from,valid_until")
        .eq("equipment_id", equipmentId)
        .eq("engineer_id", assessorEngineer.id)
        .maybeSingle();
      if (peerError || !peerCapability) return json(req, { error: "You are not a validated assessor for this equipment" }, 403);

      const today = new Date().toISOString().slice(0, 10);
      const current = peerCapability.capability_status === "ACTIVE"
        && peerCapability.validation_status === "VALIDATED"
        && (!peerCapability.valid_from || peerCapability.valid_from <= today)
        && (!peerCapability.valid_until || peerCapability.valid_until >= today);
      const authorityOkay = assessmentLevel >= 4
        ? peerCapability.practice_authority === "AUTHORISER"
        : ["INDEPENDENT", "AUTHORISER"].includes(peerCapability.practice_authority);
      if (!current || Number(peerCapability.competency_level) < assessmentLevel || !authorityOkay) {
        return json(req, { error: "Your validated equipment level or practice authority is not sufficient for this assessment" }, 403);
      }
    ...
    return json(req, { error: "Equipment competency assessment could not be saved" }, 500);
  }
});
