...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: equipment, error: equipmentError } = await db
      ...
      .select("id,site_id,name,equipment_code")
      .eq("id", equipmentId)
      .maybeSingle();
    ...
    const { data: targetEngineer, error: targetError } = await db
      .from("engineers")
    ...
    const { data: assessorEngineer } = await db
      .from("engineers")
    ...
    if (access && MANAGER_ROLES.has(role)) {
      ...
      const { data: peerCapability, error: peerError } = await db
