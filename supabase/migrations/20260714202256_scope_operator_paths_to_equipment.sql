...
begin
  ...
  left join lateral (
    ...
    where path.operator_id = operator.id
      ...
      and (
        path.equipment_id = assignment.equipment_id
        or path.equipment_id is null
      )
    order by
      case when path.equipment_id = assignment.equipment_id then 0 else 1 end,
  ...
  left join lateral (
    ...
    join public.operator_skills skill on skill.id = skill_assignment.skill_id
