...
begin
  ...
  values (v_org_id, v_site_name, nullif(btrim(p_site_location), ''), nullif(btrim(p_country), ''), v_user_id, v_user_id)
  ...
  values (v_org_id, v_site_id, v_user_id, v_user_id, 'SITE_CREATED', jsonb_build_object('authority','site_owner','portal_role','site_admin','email',lower(v_email)));
...
set owner_user_id = site.created_by
where site.owner_user_id is null
  and site.created_by is not null;
