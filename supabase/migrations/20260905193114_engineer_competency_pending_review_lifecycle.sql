alter table public.equipment_competency_assessments
  add column if not exists reviewed_by_profile_id uuid references public.profiles(id) on delete set null,
  add column if not exists reviewed_by_engineer_id uuid references public.engineers(id) on delete set null,
  add column if not exists reviewed_at timestamptz,
  add column if not exists review_outcome text,
  add column if not exists review_notes text;

alter table public.equipment_competency_assessments
  drop constraint if exists equipment_competency_assessments_status_check;
alter table public.equipment_competency_assessments
  add constraint equipment_competency_assessments_status_check
  check (assessment_status in ('pending','validated','rejected','superseded'));

alter table public.equipment_competency_assessments
  drop constraint if exists equipment_competency_assessments_review_outcome_check;
alter table public.equipment_competency_assessments
  add constraint equipment_competency_assessments_review_outcome_check
  check (review_outcome is null or review_outcome in ('validated','rejected'));

create unique index if not exists equipment_competency_one_pending_idx
  on public.equipment_competency_assessments(equipment_id, engineer_id)
  where assessment_status = 'pending';

revoke all on table public.equipment_competency_assessments from anon, authenticated;
grant all on table public.equipment_competency_assessments to service_role;

revoke update (self_rating) on table public.engineer_skills from authenticated;

create or replace function public.vorta_submit_equipment_competency_self_assessment(
  p_site_id uuid,
  p_equipment_id uuid,
  p_engineer_id uuid,
  p_profile_id uuid,
  p_assessment_level integer,
  p_evidence_reference text default null,
  p_notes text default null
)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public
as $function$
declare
  v_assessment_id uuid;
  v_previous_pending_id uuid;
begin
  if p_assessment_level < 1 or p_assessment_level > 5 then
    raise exception 'Assessment level must be between 1 and 5' using errcode = '22023';
  end if;

  if not exists (
    select 1
    from public.engineers engineer
    where engineer.id = p_engineer_id
      and engineer.profile_id = p_profile_id
      and engineer.site_id = p_site_id
  ) then
    raise exception 'Authenticated engineer identity does not match the supplied site' using errcode = '42501';
  end if;

  if not exists (
    select 1
    from public.equipment_assets equipment
    where equipment.id = p_equipment_id
      and equipment.site_id = p_site_id
  ) then
    raise exception 'Equipment is not in the supplied site' using errcode = '23514';
  end if;

  select assessment.id
  into v_previous_pending_id
  from public.equipment_competency_assessments assessment
  where assessment.equipment_id = p_equipment_id
    and assessment.engineer_id = p_engineer_id
    and assessment.assessment_status = 'pending'
  order by assessment.assessed_at desc, assessment.created_at desc
  limit 1
  for update;

  if v_previous_pending_id is not null then
    update public.equipment_competency_assessments
    set assessment_status = 'superseded'
    where id = v_previous_pending_id;
  end if;

  insert into public.equipment_competency_assessments (
    site_id,
    equipment_id,
    engineer_id,
    assessor_profile_id,
    assessor_engineer_id,
    assessment_level,
    assessment_status,
    assessor_authority,
    evidence_reference,
    notes,
    supersedes_assessment_id,
    assessed_at,
    created_at
  ) values (
    p_site_id,
    p_equipment_id,
    p_engineer_id,
    p_profile_id,
    p_engineer_id,
    p_assessment_level,
    'pending',
    'SELF',
    nullif(btrim(p_evidence_reference), ''),
    nullif(btrim(p_notes), ''),
    v_previous_pending_id,
    now(),
    now()
  ) returning id into v_assessment_id;

  return jsonb_build_object(
    'assessmentId', v_assessment_id,
    'siteId', p_site_id,
    'equipmentId', p_equipment_id,
    'engineerId', p_engineer_id,
    'assessmentLevel', p_assessment_level,
    'assessmentStatus', 'pending',
    'assessorAuthority', 'SELF',
    'submittedAt', now()
  );
end;
$function$;

revoke all on function public.vorta_submit_equipment_competency_self_assessment(uuid,uuid,uuid,uuid,integer,text,text) from public, anon, authenticated;
grant execute on function public.vorta_submit_equipment_competency_self_assessment(uuid,uuid,uuid,uuid,integer,text,text) to service_role;

create or replace function public.vorta_reject_equipment_competency_self_assessment(
  p_assessment_id uuid,
  p_reviewer_profile_id uuid,
  p_reviewer_engineer_id uuid default null,
  p_review_notes text default null
)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public
as $function$
declare
  v_assessment public.equipment_competency_assessments%rowtype;
begin
  select *
  into v_assessment
  from public.equipment_competency_assessments
  where id = p_assessment_id
  for update;

  if v_assessment.id is null then
    raise exception 'Assessment was not found' using errcode = 'P0002';
  end if;

  if v_assessment.assessment_status <> 'pending' then
    raise exception 'Assessment is no longer pending' using errcode = '23514';
  end if;

  if v_assessment.assessor_profile_id = p_reviewer_profile_id then
    raise exception 'Engineers cannot review their own equipment competency proposal' using errcode = '42501';
  end if;

  update public.equipment_competency_assessments
  set assessment_status = 'rejected',
      review_outcome = 'rejected',
      reviewed_by_profile_id = p_reviewer_profile_id,
      reviewed_by_engineer_id = p_reviewer_engineer_id,
      reviewed_at = now(),
      review_notes = nullif(btrim(p_review_notes), '')
  where id = p_assessment_id;

  return jsonb_build_object(
    'assessmentId', p_assessment_id,
    'assessmentStatus', 'rejected',
    'reviewOutcome', 'rejected',
    'reviewedAt', now()
  );
end;
$function$;

revoke all on function public.vorta_reject_equipment_competency_self_assessment(uuid,uuid,uuid,text) from public, anon, authenticated;
grant execute on function public.vorta_reject_equipment_competency_self_assessment(uuid,uuid,uuid,text) to service_role;

create or replace function public.vorta_apply_equipment_competency_assessment(
  p_site_id uuid,
  p_equipment_id uuid,
  p_engineer_id uuid,
  p_assessor_profile_id uuid,
  p_assessor_engineer_id uuid,
  p_assessment_level integer,
  p_assessor_authority text,
  p_evidence_reference text default null,
  p_notes text default null
)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public
as $function$
declare
  v_existing public.equipment_engineer_capabilities%rowtype;
  v_assessment_id uuid;
  v_capability_id uuid;
  v_previous_assessment_id uuid;
begin
  if p_assessment_level < 1 or p_assessment_level > 5 then
    raise exception 'Assessment level must be between 1 and 5' using errcode = '22023';
  end if;

  if not exists (
    select 1 from public.equipment_assets e
    where e.id = p_equipment_id and e.site_id = p_site_id
  ) then
    raise exception 'Equipment is not in the supplied site' using errcode = '23514';
  end if;

  if not exists (
    select 1 from public.engineers e
    where e.id = p_engineer_id and e.site_id = p_site_id
  ) then
    raise exception 'Engineer is not in the supplied site' using errcode = '23514';
  end if;

  select * into v_existing
  from public.equipment_engineer_capabilities c
  where c.equipment_id = p_equipment_id and c.engineer_id = p_engineer_id
  for update;

  if found and v_existing.capability_role = 'PRIMARY_SME' and p_assessment_level < 4 then
    raise exception 'Primary SME competency cannot be validated below level 4 without changing the capability role first' using errcode = '23514';
  end if;

  select id into v_previous_assessment_id
  from public.equipment_competency_assessments
  where equipment_id = p_equipment_id
    and engineer_id = p_engineer_id
  order by
    case when assessment_status = 'pending' then 0 else 1 end,
    assessed_at desc,
    created_at desc
  limit 1
  for update;

  insert into public.equipment_competency_assessments (
    site_id, equipment_id, engineer_id, assessor_profile_id, assessor_engineer_id,
    assessment_level, assessment_status, assessor_authority, evidence_reference,
    notes, supersedes_assessment_id, assessed_at, created_at
  ) values (
    p_site_id, p_equipment_id, p_engineer_id, p_assessor_profile_id, p_assessor_engineer_id,
    p_assessment_level, 'validated', p_assessor_authority, nullif(btrim(p_evidence_reference), ''),
    nullif(btrim(p_notes), ''), v_previous_assessment_id, now(), now()
  ) returning id into v_assessment_id;

  if v_previous_assessment_id is not null then
    update public.equipment_competency_assessments
    set assessment_status = case when assessment_status = 'pending' then 'superseded' else assessment_status end,
        review_outcome = case when assessment_status = 'pending' then 'validated' else review_outcome end,
        reviewed_by_profile_id = case when assessment_status = 'pending' then p_assessor_profile_id else reviewed_by_profile_id end,
        reviewed_by_engineer_id = case when assessment_status = 'pending' then p_assessor_engineer_id else reviewed_by_engineer_id end,
        reviewed_at = case when assessment_status = 'pending' then now() else reviewed_at end,
        review_notes = case when assessment_status = 'pending' then nullif(btrim(p_notes), '') else review_notes end
    where id = v_previous_assessment_id;
  end if;

  if v_existing.id is null then
    insert into public.equipment_engineer_capabilities (
      equipment_id, engineer_id, capability_role, capability_status, competency_level,
      practice_authority, validation_status, specialism, evidence_reference,
      valid_from, valid_until, notes, verified_by_profile_id, verified_by_engineer_id,
      verified_at, created_at, updated_at
    ) values (
      p_equipment_id, p_engineer_id, 'QUALIFIED_SUPPORT', 'ACTIVE', p_assessment_level,
      'SUPERVISED', 'VALIDATED', null, nullif(btrim(p_evidence_reference), ''),
      current_date, null, 'Competency validated; execution authority remains supervised until separately authorised.',
      p_assessor_profile_id, p_assessor_engineer_id, now(), now(), now()
    ) returning id into v_capability_id;
  else
    update public.equipment_engineer_capabilities
    set competency_level = p_assessment_level,
        validation_status = 'VALIDATED',
        evidence_reference = coalesce(nullif(btrim(p_evidence_reference), ''), evidence_reference),
        verified_by_profile_id = p_assessor_profile_id,
        verified_by_engineer_id = p_assessor_engineer_id,
        verified_at = now(),
        valid_from = least(valid_from, current_date),
        updated_at = now()
    where id = v_existing.id
    returning id into v_capability_id;
  end if;

  return jsonb_build_object(
    'assessmentId', v_assessment_id,
    'capabilityId', v_capability_id,
    'equipmentId', p_equipment_id,
    'engineerId', p_engineer_id,
    'assessmentLevel', p_assessment_level,
    'assessmentStatus', 'validated',
    'assessorAuthority', p_assessor_authority,
    'supersedesAssessmentId', v_previous_assessment_id,
    'verifiedAt', now()
  );
end;
$function$;

revoke all on function public.vorta_apply_equipment_competency_assessment(uuid,uuid,uuid,uuid,uuid,integer,text,text,text) from public, anon, authenticated;
grant execute on function public.vorta_apply_equipment_competency_assessment(uuid,uuid,uuid,uuid,uuid,integer,text,text,text) to service_role;
