...
begin
  ...
  v_idempotency_key := coalesce(
    ...
    md5(concat_ws(
      '|', v_actor::text, p_site_id::text, p_interaction_id::text,
      v_kind, coalesce(p_target_id::text, ''), v_changes::text
    ))
  );
...
revoke all on function public.vorta_get_ask_vorta_action_draft(uuid) from public, anon, authenticated;
grant execute on function public.vorta_get_ask_vorta_action_draft(uuid) to authenticated, service_role;
revoke all on function public.vorta_cancel_ask_vorta_action(uuid, integer) from public, anon, authenticated;
grant execute on function public.vorta_cancel_ask_vorta_action(uuid, integer) to authenticated, service_role;
